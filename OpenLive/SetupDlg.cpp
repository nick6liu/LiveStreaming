...
void CSetupDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT_MINQP, m_edtMinQP);
	DDX_Control(pDX, IDC_EDIT_MAX_QP, m_edtMaxQP);
	DDX_Control(pDX, IDC_EDIT_MAX_FRAMERATE, m_edtMinFrameRate);
	DDX_Control(pDX, IDC_CKBFRAME_SETUP, m_btnChkBFrame);
...
void CSetupDlg::InitCtrls()
{
	...
	m_cmbFPS.InsertString(0, _T("15fps"));
	m_cmbFPS.InsertString(0, _T("24fps"));
	m_cmbFPS.InsertString(0, _T("30fps"));
}
...
void CSetupDlg::OnBnClickedBtnconfirmSetup()
{
	...
	CString str;
	m_edtMinQP.GetWindowText(str);
	int minQP = _ttoi(str);
	
	m_edtMaxQP.GetWindowText(str);
	int maxQP = _ttoi(str);

	m_edtMinFrameRate.GetWindowText(str);
	int minFrameRate = _ttoi(str);


	AParameter apm(*CAgoraObject::GetAgoraObject()->GetEngine());

	char buf[512];
	sprintf_s(buf, "{\"che.video.minQP\" : %d}", minQP);
	apm->setParameters(buf);
	sprintf_s(buf, "{\"che.video.maxQP\" : %d}", maxQP);
	apm->setParameters(buf);
	sprintf_s(buf, "{\"che.video.minFramerate \" : %d}", minFrameRate);
	apm->setParameters(buf);

	if (m_btnChkBFrame.GetCheck()){
		CAgoraObject::GetAgoraObject()->SetBFrameServerIP();
	}
...
int CSetupDlg::GetScreenFPS()
{
	return m_cmbFPS.GetCurSel();
}
