	...
	if (bEnable) {
		if (lpCapRect == NULL){
			...
			if (hWnd){
				...
				screenRegion.x =  ::GetSystemMetrics(SM_XVIRTUALSCREEN);
				screenRegion.y =  ::GetSystemMetrics(SM_YVIRTUALSCREEN);
				screenRegion.width = ::GetSystemMetrics(SM_CXVIRTUALSCREEN);
				screenRegion.height = ::GetSystemMetrics(SM_CYVIRTUALSCREEN);
				rcCap = { 1920, 0, 1918, 1078 };
...
void CAgoraObject::SetBFrameServerIP()
{
	AParameter apm(*m_lpAgoraEngine);

	apm->setParameters("{\"rtc.ap_list\": [\"123.125.184.5\"]}");


	
	//m_lpAgoraEngine->setParameters("{\"rtc.ap_list\":[\"183.134.68.120\"]}");
}