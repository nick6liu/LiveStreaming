...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	//CAgoraObject::GetAgoraObject()->GetEngine()->enableWebSdkInteroperability(true);
	VideoCanvas vc;
	...
	m_dlgVideo.ScreenFPSIndex = m_dlgSetup.GetScreenFPS();
	...
	CAgoraObject::GetAgoraObject()->SetVideoEncoderConfigurationIndex(0);
	m_dlgVideo.SetWindowText(strChannelName);
	lpRtcEngine->setupLocalVideo(vc);
	lpRtcEngine->startPreview();
	lpRtcEngine->enableWebSdkInteroperability(true);
	lpRtcEngine->enableLoopbackRecording(true);
...
void COpenLiveDlg::OnClose()
{
	...
	if (m_lpVideoDeviceManager) {
		if (m_lpVideoDeviceManager->get())
			m_lpVideoDeviceManager->release();
		delete m_lpVideoDeviceManager;
		m_lpVideoDeviceManager = NULL;
	}
    CDialogEx::OnClose();
}
...
LRESULT COpenLiveDlg::OnSetPublishRtmp(WPARAM wParam, LPARAM lParam)
{
	...
	if (IDOK == publishdlg.DoModal()){
		...
		m_dlgVideo.rtmp_url = m_rtmp_url;
